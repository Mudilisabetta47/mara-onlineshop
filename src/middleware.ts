...
export function middleware(req: NextRequest) {
  ...
  if (!wantsHtml) {
    return NextResponse.json(
      { error: "Konfigurationsfehler", env: report.env, ...(detail ? { problems: report.errors } : { hint: "Details im Server-Log (Vercel → Logs, Suche: [config:)" }) },
      { status: 503, headers },
    );
  }
...
<body><main><h1>Der Shop ist nicht korrekt konfiguriert.</h1><p>Umgebung: <code>${esc(report.env)}</code> – bitte die folgenden Punkte in den Environment Variables prüfen und neu deployen.</p>${list}</main></body></html>`,
